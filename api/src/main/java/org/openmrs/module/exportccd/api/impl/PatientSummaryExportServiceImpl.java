...
public class PatientSummaryExportServiceImpl extends BaseOpenmrsService implements PatientSummaryExportService {
	...
	public ContinuityOfCareDocument produceCCD(int patientId) {
		...
		ccd = this.buildHeader(ccd, patient);
		ccd = this.buildAllergies(ccd, patient);
		ccd = this.buildProblems(ccd, patient);
		ccd = this.buildMedication(ccd, patient);
		ccd = this.buildVitalSigns(ccd, patient);
		ccd = this.buildSocialHistory(ccd, patient);
		ccd = this.buildLabResults(ccd, patient);
		ccd = this.buildPlanOfCare(ccd, patient);
		ccd = this.buildFamilyHistory(ccd, patient);
	...
	public boolean saveConceptAsCCDSections(List<Integer> concepts, String category) throws DAOException, APIException {
		for (int i = 0; i < concepts.size(); ++i) {
			this.entity = new CCDSectionEntity();
			this.entity.setConcept(Context.getConceptService().getConcept((Integer) concepts.get(i)));
			this.entity.setCategory(category);
			this.entity.setCcdSectionEntity(concepts.get(i) + category);
			CCDSectionEntity var4 = this.dao.saveConceptByCategory(this.entity);
		}
	...
	public boolean deleteConceptsByCategory(List<Concept> concepts, String category) {
		...
		while (i$.hasNext()) {
			...
			CCDSectionEntity e = this.dao.getConceptByCcdSectionEntity(concept.getId(), category);
			this.dao.deleteConceptByCategory(e);
		}
	...
	public List<Concept> getConceptByCategory(String category) {
		List<Concept> concept = this.dao.getConceptByCategory(category);
		return concept;
	}
	...
	private ContinuityOfCareDocument buildEncounters(ContinuityOfCareDocument ccd, Patient patient) {
		...
		for (Iterator i$ = encounterList.iterator(); i$.hasNext(); ++i) {
			...
			buffer.append("<td>" + this.s.format(date) + "</td>");
			buffer.append("</tr>");
	...
	ContinuityOfCareDocument buildAllergies(ContinuityOfCareDocument ccd, Patient patient) {
		...
		allergySection.getTemplateIds().add(
		    this.buildTemplateID("2.16.840.1.113883.3.88.11.83.102", (String) null, "HITSP/C83"));
		...
		for (Iterator i$ = patientAllergyList.iterator(); i$.hasNext(); ++i) {
			Allergy patientAllergy = (Allergy) i$.next();
			buffer.append("<tr>");
			buffer.append("<td><content id=\"allergy" + i + " \">" + patientAllergy.getAllergen().getDisplayString()
			        + "</content></td>");
			buffer.append("<td><content id=\"reaction" + i + " \">" + patientAllergy.getReaction().getDisplayString()
			        + "</content></td>");
			...
			allergyAct.getIds().add(this.buildID(patientAllergy.getUuid(), ""));
			...
			allergyAct.setEffectiveTime(this.buildEffectiveTimeinIVL(patientAllergy.getStartDate(),
			...
			allergyReactionObservation.getIds().add(this.buildID(patientAllergy.getReaction().getUuid(), ""));
			allergyReactionObservation.setCode(this.buildCode("59037007", "2.16.840.1.113883.6.96", "Drug Intolerance",
			    "SNOMED-CT"));
			allergyReactionObservation.setText(this.buildEDText("#reaction" + i));
			allergyReactionObservation.setEffectiveTime(this.buildEffectiveTimeinIVL(patientAllergy.getStartDate(),
			    (Date) null));
			allergyReactionObservation.getValues().add(this.buildConceptCode(patientAllergy.getAllergen()));
			...
			CE c1 = this.buildConceptCode(patientAllergy.getAllergen());
			c1.setOriginalText(this.buildEDText("#allergy" + i));
			...
			statusObservation.getTemplateIds().add(
			    this.buildTemplateID("2.16.840.1.113883.10.20.1.39", (String) null, (String) null));
			statusObservation.setCode(this.buildCode("33999-4", "2.16.840.1.113883.6.1", "Status", "LOINC"));
			...
			statusObservation.getValues().add(this.buildConceptCode(alleryStatus));
	...
	private ContinuityOfCareDocument buildHeader(ContinuityOfCareDocument ccd, Patient patient) {
		...
		Date d = new Date();
		ccd.setEffectiveTime(this.buildEffectiveTime(d));
		SimpleDateFormat s = new SimpleDateFormat("yyyyMMddhhmmss");
		String creationDate = s.format(d);
		ccd.setId(this.buildID(Context.getAdministrationService().getImplementationId().getImplementationId(),
		...
		ccd.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.27.1776", "", "CDA/R2"));
		ccd.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.3", "", "HL7/CDT Header"));
		ccd.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.32.1", "", "HITSP/C32"));
		ccd.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.1.1", "", "IHE/PCC"));
		ccd.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.150.2474.11.2.1", "", ""));
		ccd.setCode(this.buildCodeCE("422735006", "2.16.840.1.113883.6.96", "Summary clinical document", "SNOMED CT"));
		ccd.setTitle(this.buildST("Medical Summary of " + patient.getGivenName() + " " + patient.getFamilyName()
		        + "create on " + d));
		...
		patientRole.getIds().add(
		    this.buildID(Context.getAdministrationService().getImplementationId().getImplementationId(), patient
		...
		author.setTime(this.buildEffectiveTime(d));
	...
	private ContinuityOfCareDocument buildProblems(ContinuityOfCareDocument ccd, Patient patient) {
		...
		problemSection.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.103", "", "HITSP/C83"));
		problemSection.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.6", "", "IHE PCC"));
		problemSection.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.11", "", "HL7 CCD"));
		problemSection.setCode(this.buildCodeCE("11450-4", "2.16.840.1.113883.6.1", "Problem list", "LOINC"));
		problemSection.setTitle(this.buildST("Problems"));
		...
		while (i$.hasNext()) {
			...
			problemAct.getTemplateIds().add(
			    this.buildTemplateID("2.16.840.1.113883.3.88.11.83.7", (String) null, "HITSP C83"));
			problemAct.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.27", (String) null, "CCD"));
			problemAct.getTemplateIds().add(
			    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.5.2", (String) null, "IHE PCC"));
			problemAct.getTemplateIds().add(
			    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.5.1", (String) null, "IHE PCC"));
			problemAct.getIds().add(this.buildID(patientProblem.getUuid(), ""));
			problemSection.getEntries().add(problemEntry);
			problemAct.setEffectiveTime(this.buildEffectiveTimeinIVL(patientProblem.getStartDate(),
			...
			problemObservation.getTemplateIds().add(
			    this.buildTemplateID("2.16.840.1.113883.10.20.1.28", (String) null, "CCD"));
			problemObservation.getTemplateIds().add(
			    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.5", (String) null, "IHE PCC"));
			problemObservation.getIds().add(this.buildID(patientProblem.getUuid(), ""));
			problemObservation.setCode(this.buildCode("64572001", "2.16.840.1.113883.6.96", "Condition", "SNOMED-CT"));
			problemObservation.setText(this.buildEDText("#" + patientProblem.getProblem()));
			...
			problemObservation.setEffectiveTime(this.buildEffectiveTimeinIVL(patientProblem.getStartDate(),
			    patientProblem.getEndDate()));
			CE code = this.buildConceptCode(patientProblem.getProblem());
			problemObservation.getValues().add(
			    this.buildCode(code.getCode(), code.getCodeSystem(), code.getDisplayName(), code.getCodeSystemName()));
			...
			statusObservation.setClassCode(ActClassObservation.OBS);
			statusObservation.setMoodCode(x_ActMoodDocumentObservation.EVN);
			statusObservation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.50", "", ""));
			statusObservation.setCode(this.buildCode("33999-4", "2.16.840.1.113883.6.1", "Status", "LOINC"));
			...
			if (var10000.equals(ProblemModifier.HISTORY_OF)) {
				statusObservation.getValues().add(
				    this.buildCode("90734009", "2.16.840.1.113883.6.96", "Chronic", "Snomed CT"));
			} else {
				statusObservation.getValues().add(
				    this.buildCode("415684004", "2.16.840.1.113883.6.96", "Rule Out", "Snomed CT"));
			}
	...
	private ContinuityOfCareDocument buildMedication(ContinuityOfCareDocument ccd, Patient patient) {
		...
		medicationSection.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.112", "", "HITSP/C83"));
		medicationSection.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.19", "", "IHE PCC"));
		medicationSection.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.8", "", "HL7 CCD"));
		medicationSection
		        .setCode(this.buildCodeCE("10160-0", "2.16.840.1.113883.6.1", "History of medication use", "LOINC"));
		medicationSection.setTitle(this.buildST("Medication"));
		StringBuffer buffer = new StringBuffer();
		buffer.append("<table border=\"1\" width=\"100%\">");
		buffer.append("<thead>");
		buffer.append("<tr>");
		...
		for (Iterator i$ = drugOrders.iterator(); i$.hasNext(); ++i) {
			...
			medicationSubstance.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.7.1", "", "IHE PCC"));
			medicationSubstance.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.7", "", "IHE PCC"));
			medicationSubstance.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.24", "", "CCD"));
			medicationSubstance.getIds().add(this.buildID(drugOrder.getDrug().getUuid(), ""));
			CS statusCode = DatatypesFactory.eINSTANCE.createCS();
			statusCode.setCode("completed");
			...
			medicationSubstance.setText(this.buildEDText("#drug" + i));
			...
			manufacturedProduct.getTemplateIds().add(
			    this.buildTemplateID("2.16.840.1.113883.3.88.11.83.8.2", "", "HITSP C83"));
			manufacturedProduct.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.53", "", "CCD"));
			manufacturedProduct.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.7.2", "", "IHE PCC"));
			...
			CE materialCode = this.buildConceptCode(drugOrder.getConcept(), "RxNorm");
			materialCode.setOriginalText(this.buildEDText("#drug" + i));
			...
			medicationStatusObs.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.47", "", ""));
			medicationStatusObs.setCode(this.buildCode("33999-4", "2.16.840.1.113883.6.1", "Status", "LOINC"));
			medicationStatusObs.getValues().add(
			    this.buildCodeCE("55561003", "2.16.840.1.113883.6.96", "Active", "SNOMED-CT"));
	...
	private ContinuityOfCareDocument buildPlanOfCare(ContinuityOfCareDocument ccd, Patient patient) {
		...
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.124", "", "HITSP/C83"));
		section.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.31", "", "IHE PCC"));
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.2.7", "", "HL7 CCD"));
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.10", "", "HL7 CCD"));
		section.setCode(this.buildCodeCE("18776-5", "2.16.840.1.113883.6.1", "Treatment plan", "LOINC"));
		section.setTitle(this.buildST("Plan Of Care"));
		StringBuffer buffer = new StringBuffer();
		buffer.append("<table border=\"1\" width=\"100%\">");
		buffer.append("<thead>");
		buffer.append("<tr>");
			...
			while (i$.hasNext()) {
				...
				switch (type) {
					...
					default:
						break;
					case 6:
						buffer.append("<td>" + this.s.format(obs.getValueDate()) + "</td>");
						break;
					case 7:
						buffer.append("<td>" + obs.getValueTime() + "</td>");
						break;
					case 8:
						buffer.append("<td>" + this.s.format(obs.getValueDatetime()) + "</td>");
						break;
					case 10:
				...
				observation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.25", "", "HITSP C83"));
				observation.getIds().add(this.buildID(obs.getUuid(), ""));
				observation.setCode(this.buildConceptCode(obs.getConcept(), "SNOMED", "LOINC"));
				observation.setText(this.buildEDText("#" + obs.getConcept().getDisplayString()));
				CS statusCode1 = DatatypesFactory.eINSTANCE.createCS();
				statusCode1.setCode("new");
				observation.setStatusCode(statusCode1);
				observation.setEffectiveTime(this.buildEffectiveTimeinIVL(obs.getObsDatetime(), (Date) null));
				EntryRelationship entryRelationship = CDAFactory.eINSTANCE.createEntryRelationship();
				...
				act.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.4.1", "", ""));
				if (obs.getEncounter() != null) {
					act.getIds().add(this.buildID(obs.getEncounter().getUuid(), ""));
				}
	...
	private ContinuityOfCareDocument buildVitalSigns(ContinuityOfCareDocument ccd, Patient patient) {
		...
		ccd.addSection(section);
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.119", "", "HITSP/C83"));
		section.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.1.5.3.2", "", "IHE PCC"));
		section.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.25", "", "IHE PCC"));
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.16", "", "HL7 CCD"));
		section.setCode(this.buildCodeCE("8716-3", "2.16.840.1.113883.6.1", "Vital signs", "LOINC"));
		section.setTitle(this.buildST("Vital Signs"));
		StringBuffer buffer = new StringBuffer();
		buffer.append("<table border=\"1\" width=\"100%\">");
		buffer.append("<thead>");
		buffer.append("<tr>");
		buffer.append("<th>Date</th>");
		List<Concept> vitalSignsList = this.dao.getConceptByCategory("VitalSigns");
		List<Obs> listOfObservations = new ArrayList();
		...
		while (true) {
			...
			while (i$.hasNext()) {
				...
				if (vitalSignData.containsKey(date)) {
					String data = (String) vitalSignData.get(date);
					data = data + "," + obs.getId();
			...
			while (i$.hasNext()) {
				date = (Date) i$.next();
				buffer.append("<th>" + this.s.format(date) + "</th>");
			}
			...
			while (i$.hasNext()) {
				...
				while (i$.hasNext()) {
					...
					if (vitalSignData.containsKey(date + "" + concept)) {
						...
						String obsId = (String) vitalSignData.get(date + "" + concept);
						...
						organizer.getTemplateIds().add(
						    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.13.1", "", "IHE PCC"));
						organizer.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.32", "", "CCD"));
						organizer.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.35", "", "CCD"));
						organizer.getIds().add(this.buildID(obs.getUuid(), ""));
						organizer.setCode(this.buildCode("46680005", "2.16.840.1.113883.6.96", "Vital signs", "SNOMED-CT"));
						CS statusCode = DatatypesFactory.eINSTANCE.createCS();
						statusCode.setCode("completed");
						organizer.setStatusCode(statusCode);
						organizer.setEffectiveTime(this.buildEffectiveTimeinIVL(date, (Date) null));
						Component4 component = CDAFactory.eINSTANCE.createComponent4();
						Observation observation = CDAFactory.eINSTANCE.createObservation();
						observation.setClassCode(ActClassObservation.OBS);
						observation.setMoodCode(x_ActMoodDocumentObservation.EVN);
						observation.getTemplateIds().add(
						    this.buildTemplateID("2.16.840.1.113883.3.88.11.83.14", "", "HITSP C83"));
						observation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.31", "", "CCD"));
						observation.getTemplateIds().add(
						    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.13", "", "IHE PCC"));
						observation.getIds().add(this.buildID(obs.getUuid(), ""));
						observation.setCode(this.buildConceptCode(concept, "SNOMED", "LOINC"));
						observation.setText(this.buildEDText("#" + concept.getDisplayString()));
						CS statusCode1 = DatatypesFactory.eINSTANCE.createCS();
						statusCode1.setCode("completed");
						observation.setStatusCode(statusCode1);
						observation.setEffectiveTime(this.buildEffectiveTimeinIVL(obs.getObsDatetime(), (Date) null));
	...
	private ContinuityOfCareDocument buildLabResults(ContinuityOfCareDocument ccd, Patient patient) {
		...
		ccd.addSection(section);
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.122", "", "HITSP/C83"));
		section.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.28", "", "IHE PCC"));
		section.setCode(this.buildCodeCE("30954-2", "2.16.840.1.113883.6.1", "Relevant diagnostic tests/laboratory data",
		    "LOINC"));
		section.setTitle(this.buildST("Results"));
		StringBuffer buffer = new StringBuffer();
		buffer.append("<table border=\"1\" width=\"100%\">");
		buffer.append("<thead>");
		buffer.append("<tr>");
		buffer.append("<th>Date</th>");
		List<Concept> labResultsList = this.dao.getConceptByCategory("LabResults");
		List<Obs> listOfObservations = new ArrayList();
			...
			while (i$.hasNext()) {
				...
				while (ii$.hasNext()) {
					...
					if (labResultData.containsKey(date + "" + concept)) {
						...
						organizer.setClassCode(x_ActClassDocumentEntryOrganizer.CLUSTER);
						organizer.setMoodCode(ActMood.EVN);
						organizer.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.32", "", ""));
						organizer.getIds().add(this.buildID(obs.getUuid(), ""));
						organizer.setCode(this.buildCode("56850-1", "2.16.840.1.113883.6.1",
						...
						observation.setMoodCode(x_ActMoodDocumentObservation.EVN);
						observation.getTemplateIds().add(
						    this.buildTemplateID("2.16.840.1.113883.3.88.11.83.15", "", "HITSP C83"));
						observation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.31", "", "CCD"));
						observation.getTemplateIds().add(
						    this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.13", "", "IHE PCC"));
						observation.getIds().add(this.buildID(obs.getUuid(), ""));
	...
	private ContinuityOfCareDocument buildSocialHistory(ContinuityOfCareDocument ccd, Patient patient) {
		...
		ccd.addSection(section);
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.15", "", "HITSP/C83"));
		section.setCode(this.buildCodeCE("29762-2", "2.16.840.1.113883.6.1", "Social History", "LOINC"));
		section.setTitle(this.buildST("Social History"));
		StrucDocText details = CDAFactory.eINSTANCE.createStrucDocText();
		StringBuffer buffer = new StringBuffer();
		buffer.append("<table border=\"1\" width=\"100%\">");
		buffer.append("<thead>");
		...
		while (i$.hasNext()) {
			...
			buffer.append("</td>");
			buffer.append("<td>" + this.s.format(obs.getObsDatetime()) + "</td>");
			buffer.append("</tr>");
			...
			observation.setClassCode(ActClassObservation.OBS);
			observation.setMoodCode(x_ActMoodDocumentObservation.EVN);
			observation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.33", "", ""));
			observation.getIds().add(this.buildID(obs.getUuid(), ""));
			observation.setCode(this.buildConceptCode(obs.getConcept(), "SNOMED", "AMPATH", "LOINC"));
			CS statusCode = DatatypesFactory.eINSTANCE.createCS();
			statusCode.setCode("completed");
			observation.setStatusCode(statusCode);
			observation.setEffectiveTime(this.buildEffectiveTimeinIVL(obs.getObsDatetime(), (Date) null));
			ST value1 = this.buildST(value);
			...
			eObservation.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.41", "", ""));
			eObservation.setCode(this.buildCode("ASSERTION", "2.16.840.1.113883.5.4", "", ""));
			CS statusCode1 = DatatypesFactory.eINSTANCE.createCS();
			statusCode1.setCode("completed");
			eObservation.setStatusCode(statusCode1);
			CD value2 = this.buildCode("404684003", "2.16.840.1.113883.6.96", "Clinical Finding", "");
			...
			qualifier.setValue(this.buildCode("288527008", "2.16.840.1.113883.6.96", "New episode", ""));
			...
			eObservation2.getIds().add(this.buildID(obs.getConcept().getUuid(), ""));
			eObservation2.setCode(this.buildConceptCode(obs.getConcept(), "SNOMED"));
	...
	private ContinuityOfCareDocument buildFamilyHistory(ContinuityOfCareDocument ccd, Patient patient) {
		...
		ccd.addSection(section);
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.125", "", "HITSP/C83"));
		section.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.4", "", "CCD"));
		section.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.3.14", "", "IHE PCC"));
		section.setCode(this.buildCodeCE("10157-6", "2.16.840.1.113883.6.1", "History of family member diseases", "LOINC"));
		section.setTitle(this.buildST("Family History"));
		StrucDocText details = CDAFactory.eINSTANCE.createStrucDocText();
		StringBuffer buffer = new StringBuffer();
		...
		while (i$.hasNext()) {
			...
			organizer.setMoodCode(ActMood.EVN);
			organizer.setClassCode(x_ActClassDocumentEntryOrganizer.CLUSTER);
			organizer.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.3.88.11.83.18", "", "HITSP C83"));
			organizer.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.23", "", "CCD"));
			organizer.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.15", "", "IHE PCC"));
			CS statusCode = DatatypesFactory.eINSTANCE.createCS();
			statusCode.setCode("completed");
			organizer.setStatusCode(statusCode);
			...
			relatedSubject.setCode(this.buildConceptCode(cs.getConcept(relation), "Snomed"));
			...
			co.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.22", "", "CCD"));
			co.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.13", "", "IHE PCC"));
			co.getTemplateIds().add(this.buildTemplateID("1.3.6.1.4.1.19376.1.5.3.1.4.13.3", "", "IHE PCC"));
			co.getIds().add(this.buildID(obs.getUuid(), ""));
			co.setCode(this.buildConceptCode(cs.getConcept(diagnosis), "SNOMED"));
			co.setText(this.buildEDText("#" + diagnosis));
			CS statusCode1 = DatatypesFactory.eINSTANCE.createCS();
			statusCode1.setCode("completed");
			...
			ob.getTemplateIds().add(this.buildTemplateID("2.16.840.1.113883.10.20.1.38", "", ""));
			ob.setCode(this.buildCode("397659008", "2.16.840.1.113883.6.96", "Age", "SNOMED-CT"));
